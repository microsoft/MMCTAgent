...
async def process_v4_query(body: dict, request_id: str = "") -> Dict[str, Any]:
    ...
    logger.info(f"[{request_id}] V4 Query: {query}")
    if video_id:
    ...
    try:
        ...
        logger.error(f"[{request_id}] V4 query failed: {e}")
        raise HTTPException(500, f"V4 query processing failed: {str(e)}")


async def process_v4_query_stream(body: dict, request_id: str = "") -> AsyncGenerator[str, None]:
    ...
    def save_events():
        if save_logs and events:
            ...
            logger.info(f"[{request_id}] V4 query events saved to {log_file}")
    ...
    try:
        ...
        logger.error(f"[{request_id}] V4 streaming failed: {e}")
