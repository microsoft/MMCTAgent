...
@register_step("chapters.timeline-summary")
class ChapterTimelineSummaryStep(PipelineStep):
    """Creates an overlapping, timestamped global summary from detailed chapters."""

    description = (
        "Aggregates chapter summaries into ~10 minute windows and crafts a 4k-token global narrative."
    )

    def run(self, context: StepContext) -> StepResult:
        config = self._build_config()
        slices = self._load_chapters(context, config.chapters_step)
        if not slices:
            raise ValueError(
                f"Step '{self.step_id}' did not find any chapters in step '{config.chapters_step}'."
            )

        windows = self._build_windows(slices, config)
        if not windows:
            raise ValueError("Unable to create windows for global summary generation.")
        ...
        # Persist summary only in this step's namespace + metadata to avoid mutating chapter payloads.
        context.metadata["global_summary"] = global_summary
        ...
        metrics = {
            "chapter_windows": float(len(windows)),
            "window_span_seconds": float(config.window_seconds),
            "window_overlap_seconds": float(config.overlap_seconds),
            "approx_summary_tokens": float(approx_tokens),
        }
        return StepResult(step_id=self.step_id, produced=produced_payload, metrics=metrics)

    def _build_config(self) -> SummaryConfig:
        chapters_step = str(self.params.get("chapters_step") or "").strip()
        if not chapters_step:
            raise ValueError("'chapters_step' parameter is required")

        window_minutes = float(self.params.get("window_minutes", DEFAULT_WINDOW_MINUTES))
        if window_minutes <= 0:
            raise ValueError("'window_minutes' must be positive")
        overlap_minutes = float(self.params.get("window_overlap_minutes", DEFAULT_OVERLAP_MINUTES))
        window_seconds = window_minutes * 60.0
        overlap_seconds = max(0.0, min(window_seconds - 1.0, overlap_minutes * 60.0))

        token_budget = int(self.params.get("target_token_budget", DEFAULT_TOKEN_BUDGET))
        if token_budget <= 0:
    ...
    def _load_chapters(self, context: StepContext, chapters_step: str) -> List[ChapterSlice]:
        payload = context.data_store.get(chapters_step, {}) or {}
    ...
    def _build_windows(self, slices: List[ChapterSlice], config: SummaryConfig) -> List[ChapterWindow]:
        if not slices:
            return []
