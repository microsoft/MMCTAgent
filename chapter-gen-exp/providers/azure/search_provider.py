...
class AzureSearchProvider(SearchProvider):
    ...
    def _create_search_client(self, index_name: str) -> SearchClient:
        ...
        try:
            ...
            if not endpoint:
                raise RuntimeError("SEARCH_ENDPOINT environment variable not set")
            ...
            if use_managed_identity:
                ...
                if not api_key:
                    raise RuntimeError("Azure AI Search API key is required when managed identity is disabled")
            ...
            raise RuntimeError(f"Failed to initialize Azure AI Search client: {e}")
    ...
    def _initialize_index_client(self) -> SearchIndexClient:
        ...
        try:
            endpoint = self.config.get("endpoint")
            if not endpoint:
                raise RuntimeError("Azure AI Search endpoint is required")

            return SearchIndexClient(endpoint=endpoint, credential=self.credential)
        except Exception as e:
            raise RuntimeError(f"Failed to initialize Azure AI Search Index client: {e}")
    ...
    def _create_object_collection_index_schema(self, index_name: str, dim: int = 1536) -> SearchIndex:
        """Create Azure AI Search index schema for merged object collections."""
            ...
            if name == "video_summary_embedding":
                fields.append(
                    SearchField(
                        ...
                        hidden=not extra.get("stored", True),
                        vector_search_dimensions=dim,
                        vector_search_profile_name="object-embedding-profile",
                    )
                )
                continue
        ...
        semantic_fields = [
            ...
            SemanticField(field_name="object_collection"),
        ]
        semantic_config = SemanticSearch(
            configurations=[
                SemanticConfiguration(
                    name="object-collection-semantic-config",
                    prioritized_fields=SemanticPrioritizedFields(
                        content_fields=semantic_fields,
                        keywords_fields=semantic_fields,
                    ),
                )
            ]
        )
        ...
        vector_search = VectorSearch(
            ...
            profiles=[
                VectorSearchProfile(
                    name="object-embedding-profile",
                    algorithm_configuration_name="object-hnsw",
                )
            ],
        )
    ...
    async def search(self, query: str, index_name: str = None, **kwargs) -> List[Dict]:
        ...
        try:
            ...
            raise RuntimeError(f"Azure AI Search failed: {e}")
    ...
    async def index_document(self, document: Dict, index_name: str = None) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Azure AI Search indexing failed: {e}")
    ...
    async def delete_document(self, doc_id: str, index_name: str = None) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Azure AI Search deletion failed: {e}")


    async def delete_documents(self, document_ids: List[str], index_name: str = None) -> Dict[str, Any]:
        """
        Delete multiple documents from Azure AI Search.
        ...
        Returns:
            Dict with deletion results including success count and failed IDs
        """
        try:
            if not document_ids:
            ...
            failed_ids = [document_ids[i] for i, r in enumerate(result) if not r.succeeded]
            
            logger.info(f"Successfully deleted {success_count}/{len(document_ids)} documents from index")
            if failed_ids:
                logger.warning(f"Failed to delete {len(failed_ids)} documents: {failed_ids}")
            ...
            logger.error(f"Azure AI Search bulk deletion failed: {e}")
            raise RuntimeError(f"Azure AI Search bulk deletion failed: {e}")
    ...
    async def create_index(self, index_name: str, index_schema: Any) -> bool:
        ...
        try:
            ...
            if isinstance(index_schema, str):
                ...
                elif index_schema in {"object-collection", "object_collection"}:
                    index_schema = self._create_object_collection_index_schema(index_name)
                else:
                    raise RuntimeError(f"Unknown index schema type: {index_schema}")
                ...
                elif schema_type in {"object-collection", "object_collection"}:
                    dim = index_schema.get("dim", 1536)
                    index_schema = self._create_object_collection_index_schema(index_name, dim)
                else:
                    raise RuntimeError(f"Unknown index schema type: {schema_type}")
    ...
    async def index_exists(self, index_name: str) -> bool:
        ...
        try:
            ...
            if any(pattern in error_str for pattern in not_found_patterns):
                ...
                raise RuntimeError(f"Error checking if index '{index_name}' exists: {e}")
    ...
    async def upload_documents(self, documents: List[Dict], index_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Azure AI Search bulk upload failed: {e}")
    ...
    async def check_is_document_exist(self, hash_id: str, index_name: str = None) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to check if document exists: {e}")
