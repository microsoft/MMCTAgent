...
class AzureStorageProvider(StorageProvider):
        ...
        if self.service_client is None:
            try:
                ...
                if use_managed_identity:
                    ...
                    raise RuntimeError("Non-managed identity auth not yet implemented for blob storage")

                account_url = self.config.get("account_url") or os.getenv("STORAGE_ACCOUNT_URL")
                if not account_url:
                    raise RuntimeError("Azure Storage account_url is required")
    ...
    async def load_file_to_memory(self, folder: str, file_name: str) -> bytes:
        ...
        try:
            ...
            raise RuntimeError(f"Error loading file {file_name} into memory: {e}")
        finally:
            if client:
                await client.close()
    ...
    async def save_file(self, file_name: str, src_file_path: str, **kwargs) -> str:
        ...
        try:
            ...
            url = f"{self.service_client.url.strip('/')}/{folder_name}/{file_name}"
            return url
        except Exception as e:
            logger.exception(f"Error uploading file {src_file_path}: {e}")
            raise RuntimeError(f"Error uploading file {src_file_path}: {e}")
        finally:
            if client:
                await client.close()
    ...
    async def save_base64(self, file_name: str, b64_str: str, **kwargs) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Error uploading base64 data: {e}")
        finally:
            if client:
                await client.close()
    ...
    async def save_string(self, file_name: str, content: str, **kwargs) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Error uploading string content: {e}")
        finally:
            if client:
                await client.close()
