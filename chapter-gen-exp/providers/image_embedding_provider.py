from providers.base import ImageEmbeddingProvider
from settings import ImageEmbeddingConfig
...
class CustomImageEmbeddingProvider(ImageEmbeddingProvider):
    ...
    async def image_embedding(self, image: Union[str, Image.Image], **kwargs) -> List[float]:
        ...
        try:
            ...
            if img is None:
                raise RuntimeError("Failed to load or preprocess image")
            ...
            raise RuntimeError(f"CLIP image embedding failed: {e}")
    ...
    async def batch_image_embedding(self, images: List[Union[str, Image.Image]], **kwargs) -> List[List[float]]:
        ...
        try:
            ...
            if not processed_images:
                raise RuntimeError("No valid images to process in batch")
            ...
            raise RuntimeError(f"CLIP batch image embedding failed: {e}")
    ...
    async def batch_text_embedding(self, texts: List[str], **kwargs) -> List[List[float]]:
        ...
        try:
            ...
            raise RuntimeError(f"CLIP batch text embedding failed: {e}")
