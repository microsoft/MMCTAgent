...
from .base import LLMProvider, SearchProvider, TranscriptionProvider, VisionProvider
...
class AzureLLMProvider(LLMProvider):
    ...
    async def embedding(self, text: str, **kwargs) -> List[float]:
        ...
        try:
            deployment_name = self.config.get("embedding_deployment_name")
            if not deployment_name:
                raise ConfigurationException("Azure OpenAI embedding deployment name is required")
