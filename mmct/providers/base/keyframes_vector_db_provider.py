...
class BaseKeyframesVectorDBProvider(ABC):
    """Abstract base class for Keyframe - Vector DB providers."""
    ...
    def get_index_schema(self) -> Any:
        """
        Creates provider-specific schema based on KeyframeDocument type.
    ...
    def parse_response(self, vector_db_document: Any) -> KeyframeDocument:
        """
        Parses the retrieved vector DB document into KeyframeDocument object.

        Args:
            vector_db_document: Provider-specific document response

        Returns:
            KeyframeDocument: Parsed document
        """
        pass
    ...
    async def search(self, query: str, **kwargs) -> List[Tuple[KeyframeDocument, float]]:
        ...
        Returns:
            List of tuples containing (KeyframeDocument, similarity_score)
        """
        pass
