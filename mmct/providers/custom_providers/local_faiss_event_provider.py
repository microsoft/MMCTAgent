"""Local FAISS-based event vector database provider."""
...
class LocalFAISSEventProvider(BaseEventVectorDBProvider):
    """Local FAISS-backed event vector database provider.
    
    Stores event documents with vector embeddings using FAISS for similarity search.
    ...
    def __init__(
        ...
        """Initialize the FAISS event provider.
    ...
    def _index_file(self) -> str:
        ...
        return os.path.join(self.base_path, f"{self.index_name}_events.index")
    
    def _meta_file(self) -> str:
        """Get path to metadata JSON file."""
        return os.path.join(self.base_path, f"{self.index_name}_events.meta.json")
    ...
    async def search(
        ...
        **kwargs
        ...
        """Search events by text query with metadata filtering."""
        ...
        for doc_id, doc in self._meta["docs"].items():
            if video_id and doc.get("video_id") != video_id:
                continue
            if event_type and doc.get("event_type") != event_type:
                continue
    ...
    async def search_similar_events(
        ...
        **kwargs
        ...
        doc = self._meta["docs"].get(event_id)
        if not doc:
            return []
        ...
        return [r for r in results if r["id"] != event_id][:limit]
    ...
    async def close(self) -> None:
        ...
        await asyncio.to_thread(self._save_sync)
        logger.info(f"LocalFAISSEventProvider closed for index: {self.index_name}")
