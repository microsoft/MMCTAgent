...
class LocalFaissSearchProvider(SearchProvider):
    ...
    async def upload_clip_documents(self, documents: List[Dict], index_name: str = None) -> Dict[str, Any]:
        """
        Upload CLIP embedding documents to the index.
        
        Args:
            documents: List of documents with 'embeddings' field (from KeyframeDocument)
            index_name: Name of the index
            
        Returns:
            Dict with success status, count, and failed count
        ...
        for document in documents:
            try:
                ...
                # Get embeddings from document
                embeddings = document.get("embeddings")
                if not embeddings:
                    logger.warning(f"Document missing 'embeddings' field: {document.get('id', 'unknown')}")
                    failed += 1
                    continue
                ...
                logger.error(f"Failed to index CLIP document: {e}")
                failed += 1

        logger.info(f"Successfully uploaded {success}/{len(documents)} CLIP documents (failed: {failed})")
        return {"success": True, "count": success, "failed": failed}

    @handle_exceptions(retries=3, exceptions=(Exception,))
    @convert_exceptions({Exception: ProviderException})
    async def search_clip(self, query: str, index_name: str = None, **kwargs) -> List[Dict]:
        """
        Perform vector search using CLIP embeddings.

        Args:
            query: Search query string (used for fallback text search)
            index_name: Name of the index to search
            **kwargs: Additional parameters:
                - embedding: Query CLIP embedding for vector search
                - top: Number of results to return (default: 5)
        ...
        try:
            ...
            num_docs = len(meta.get("docs", {})) if meta else 0
            logger.debug(
                f"CLIP search on index '{index_name}': dim={meta.get('dim') if meta else None}, "
                f"docs={num_docs}, has_index={idx is not None}, has_embedding={embedding is not None}"
            )
            ...
            # Fallback: text-based substring search with additional fields for CLIP
            text_fields = ["detailed_summary", "text_from_scene", "chapter_transcript", "description"]
            return self._perform_text_search(meta, query, text_fields)
            
        except Exception as e:
            logger.error(f"Local FAISS CLIP search failed: {e}")
            raise ProviderException(f"Local FAISS CLIP search failed: {e}")
