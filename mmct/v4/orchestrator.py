...
def _print_message_to_console(message, token_usage: TokenUsage, request_id: str = "") -> None:
    ...
    Args:
        ...
        request_id: Optional request ID prefix for log correlation.
    """
    ...
    if isinstance(message, TaskResult):
        print(
            f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.MAGENTA}[System]{Colors.RESET}: Task Completed.",
            flush=True,
        )
        ...
        print(
            f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.GREEN}[{source}]{Colors.RESET}: {message.content}",
            flush=True,
        )
    elif isinstance(message, HandoffMessage):
        print(
            f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.CYAN}[{source}]{Colors.RESET}: Handoff → {Colors.BOLD}{message.target}{Colors.RESET}",
            flush=True,
        )
        if message.content:
        ...
        if isinstance(content, list):
            for tc in content:
                print(
                    f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.BLUE}[{source}]{Colors.RESET}: Tool Call: {Colors.BOLD}{tc.name}{Colors.RESET}",
                    flush=True,
                )
                print(f"{Colors.GRAY}{rid}  Args: {tc.arguments}{Colors.RESET}", flush=True)
        else:
            print(
                f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.BLUE}[{source}]{Colors.RESET}: Tool Call: {content}",
                flush=True,
            )
        ...
        if isinstance(content, list):
            for tr in content:
                print(
                    f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.YELLOW}[{source}]{Colors.RESET}: Tool Result ({tr.call_id}):",
                    flush=True,
                )
                print(f"{Colors.GRAY}{rid}{tr.content}{Colors.RESET}", flush=True)
        else:
            print(
                f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.YELLOW}[{source}]{Colors.RESET}: Tool Result: {content}",
                flush=True,
            )
    else:
        content = str(getattr(message, "content", str(message)))
        print(
            f"\n{Colors.GRAY}[{now}]{Colors.RESET} {rid}{Colors.RED}[{source}]{Colors.RESET}: [{type(message).__name__}] {content}",
            flush=True,
        )

    # Print token usage for messages that have it
    if (
        hasattr(message, "models_usage")
        and message.models_usage
        and not isinstance(message, TaskResult)
    ):
        print(
            f"{Colors.GRAY}{rid}[Tokens: +{message.models_usage.prompt_tokens} prompt, +{message.models_usage.completion_tokens} completion]{Colors.RESET}",
            flush=True,
        )
...
class V4Orchestrator:
    ...
    def _build_swarm(self) -> Swarm:
        ...
        termination = FunctionCallTermination(SUBMIT_TOOL_NAME) | TextMentionTermination(
            _TERMINATE_STRING
        )
    ...
    async def query(
        ...
        now = _format_timestamp()
        print(f"\n{'=' * 60}", flush=True)
        print(f"{rid}{Colors.CYAN}V4 Query Pipeline{Colors.RESET}", flush=True)
    ...
    async def query_stream(
        ...
        now = _format_timestamp()
        print(f"\n{'=' * 60}", flush=True)
        print(f"{rid}{Colors.CYAN}V4 Query Pipeline (Streaming){Colors.RESET}", flush=True)
        print(f"{'=' * 60}", flush=True)
        print(f"{Colors.GRAY}[{now}]{Colors.RESET} {rid}Query: {user_query}", flush=True)
        if video_id:
            print(f"{Colors.GRAY}[{now}]{Colors.RESET} {rid}Video: {video_id}", flush=True)
        ...
        try:
            async for message in team.run_stream(task=task):
                ...
                if isinstance(message, TaskResult):
                    ...
                    print(f"\n{'=' * 60}", flush=True)
                    print(
                        f"{rid}{Colors.GREEN}Query completed in {elapsed:.2f}s{Colors.RESET}",
                        flush=True,
                    )
                    print(f"{'=' * 60}\n", flush=True)
