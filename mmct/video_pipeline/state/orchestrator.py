...
Drives the pipeline through explicit states, calling LLMs only where
intelligence is needed and executing tools programmatically.
"""
...
from mmct.video_pipeline.state._helpers import format_evidence_compact, dedup_sources
...
from mmct.video_pipeline.state.prompts.context_expansion import (
    EXPAND_CONTEXT_PROMPT,
    ContextExpansionResult,
    build_expansion_user_message,
)
from mmct.video_pipeline.state.tools.retrieval import RetrievalExecutor
from mmct.video_pipeline.state.tools.video_discovery import VideoDiscoveryExecutor
from mmct.video_pipeline.state.tools.image_analysis import ImageAnalysisExecutor
from mmct.video_pipeline.state.query.neo4j_provider import Neo4jQueryProvider
from mmct.video_pipeline.state.agents.planner_agent import PlannerAgent
from mmct.video_pipeline.state.agents.critic_agent import CriticAgent
...
class StateOrchestrator:
    ...
    Args:
        ...
        use_critic: Whether to run the critic evaluation step.
        video_catalog: Optional catalog string passed to the planner.
        max_turns: Accepted for interface parity with GraphOrchestrator; ignored.
    """
    ...
    async def query(
        ...
        Args:
            ...
            video_id: Single video scope (mutually exclusive with video_ids).
            video_ids: Multi-video scope list.
            request_id: Caller-provided request identifier for log tracing.

        Returns:
            Dict with keys: answer, sources, token_usage, elapsed_seconds.
        """
        start = time.time()
    ...
    async def query_stream(
        ...
        Yields dicts of {"type": "message", ...} per state, then
        {"type": "final", "data": {...}} with the full response.
        """
        start = time.time()
        rid = request_id or "no-rid"
...
async def process_query(
    ...
    **orchestrator_kwargs,
    ...
    """Convenience function: create a StateOrchestrator and run one query."""
